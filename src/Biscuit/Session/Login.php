...
use Biscuit\Util\Time;
use Biscuit\Util\Validate;
use Biscuit\Util\Gravatar;
...
class Login {
    ...
    public function __construct( Session $session )
    {
        ...
        $this->unique  = Client::getHash();

        if( $this->session->active() !== true )
        {
            throw new Exception( 'Session needs to be started before using the Auth class.' );
        }
    ...
    {
        $text    = Utils::getValue( $text, 'Too many failed attempts.' );
        $count   = $this->session->get( $this->key.'.info.fail_count', 0 );
        $total   = $this->session->get( $this->key.'.info.fail_total', 0 );
        $last    = $this->session->get( $this->key.'.info.last_attempt', 0 );
        ...
        $keymap  = array(
            'attempts'  => $total,
            'countdown' => Time::toCountdown( $last, $this->cd_time ),
            'count'     => '('.$count.'/'.$this->cd_trigger.')',
            'total'     => '('.$total.'/'.$this->max_attempts.')',
        );
        return (string) Text::set( $text )->mapReplace( $keymap )->singleSpaces();
    }
    ...
    {
        $user = $this->session->get( $this->key.'.user', array() );
    ...
    public function login( $data=array() )
    {
        ...
        if( !empty( $data ) && is_array( $data ) )
        {
            ...
            {
                $data['image'] = Gravatar::getUrl( $data['email'] );
            }
            if( !empty( $data['options'] ) && is_string( $data['options'] ) )
            {
                $data['options'] = Json::decode( $data['options'] );
            }
     ...
     */
    public function isOwner( $entry=array(), $column='admin_id' )
    {
