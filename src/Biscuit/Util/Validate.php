...
class Validate {
    ...
    {
        return preg_match( '/^(?=.{2,})([\p{L}\'\-\ ]+)$/ui', $value );
    }
    ...
    {
        return ( is_numeric( $value ) || preg_match( '/^[0-9\-\,\.]+$/i', $value ) );
    }
    ...
    {
        return is_int( is_numeric( $value ) ? $value + 0 : $value );
    }
    ...
    {
        return is_float( is_numeric( $value ) ? $value + 0 : $value );
    }
    ...
    {
        return ( is_bool( $value ) || preg_match( '/^(Y|N|ON|OFF|YES|NO|TRUE|FALSE)$/ui', $value ) );
    }
    ...
    {
        return preg_match( '/^(\d{5}(\-\d{4})?)$/i', $value );
    }
    ...
    {
        return preg_match( '/^([1+-.]{2,3})?\(?([\d]{3})\)?[-. ]?([\d]{3})[-. ]?([\d]{4})$/', $value );
    }
    ...
    {
        return preg_match( '/^([\d\p{L}\.\_\-\!\#\$\%\&\+\=]+)@([\d\p{L}\-\.]+)(\.[\p{L}]{2,63})$/u', $value );
    }
    ...
    {
        return preg_match( '/^(?=.{2,15}$)(@){1}([\w]+)$/u', $value );
    }
    ...
    {
        return preg_match( '/^(?=.{2,140}$)(#|\x{ff03}){1}([\d\p{L}\_]*[_\p{L}][\d\p{L}\_]*)$/u', $value );
    }
    ...
    {
        return preg_match( '/^(?=.{6,})(?=.*[0-9])(?=.*[\p{L}]).*$/u', $value );
    }
    ...
    public static function isType( $value=null, $type='' )
    {
        if( !empty( $type ) )
        {
            ...
            if( is_object( $value ) && $type !== 'object' )
            {
                ...
                {
                    return ( is_callable( $value ) || $closure );
                }
            }
            if( is_string( $value ) && $type !== 'string' )
            {
                ...
                {
                    return ( function_exists( $value ) || is_callable( $value ) );
                }
                if( preg_match( '/^(file|link|symlink)$/', $type ) )
                {
                    return ( is_file( $value ) || is_link( $value ) );
                }
